...
###   This script attempts to download the signature file SHA256SUMS.asc from lmcoin.org
...
WORKINGDIR="/tmp/lmcoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://lmcoin.org/bin/"
VERSIONPREFIX="lmcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'lmcoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Lmcoin Core binary release signing key installed?"
   fi
