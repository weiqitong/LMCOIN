Lmcoin Core version 0.11.0 is now available from:

  <https://lmcoin.org/bin/lmcoin-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Lmcoin Core or other software:
...
defaults to 0.00001. This will cause transactions with fewer LMC/kB fee to be
...
For example, add the following to `lmcoin.conf`:
...
related to the blockchain in the lmcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Lmcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Lmcoin Core will 
...
For an example script see [lmcoin-submittx](https://github.com/laanwj/lmcoin-submittx).
...
- #5334 `cf87536` liblmcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new lmcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only LmcoinAmountField
- #5753 `0f44672` Add lmcoin logo to about screen
...
- #5809 `46bfbe7` Add lmcoin-cli man page
...
- #5849 `41259ca` contrib/init/lmcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Lmcoin-Qt.app to Lmcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for lmcoin-tx)
